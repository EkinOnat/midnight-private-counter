...
// Identifier under which this contract's local witness state is stored.
...
const compiledContract = CompiledContract.make('private-counter', CounterContract).pipe(
...
async function createProviders(walletCtx: WalletContext) {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'private-counter-state',
      accountId,
...
async function main() {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      // Midnight.js 4.1.x supplies local witness state via privateStateId and
      // initialPrivateState. The nonce stays local; only its Compact hash can
      // become public when increment() is called.
      const initialPrivateState: CounterPrivateState = {
        nonces: [crypto.randomBytes(32)],
        nextNonceIndex: 0,
      };
      deployed = await deployContract(providers, {
        // Provider factory is dynamic across network presets, so keep the
        // scaffold's narrow boundary cast at the SDK deployment call.
  ...
  console.log('  Next: connect a UI or CLI that provisions a fresh nonce and calls increment().\n');
}
